...
Aside from the convenience of not needing to copy and paste between different source documents, an R Notebook fully encapulates your work and is easily shareable as reproducible research [@peng2014TheImportant]. By sharing your R Notebook with collaborators or colleagues (or even sceptics!) they can take your data, code, and text, and reproduce the exact same analysis as you conducted.
...
Run a code chunk by clicking the *Run Current Chunk* button (green arrow) within the chunk or by placing your cursor inside the chunk and pressing *Ctrl + Shift + Enter*.
...
~~Strikethrough~~ is appied using double tildes: `~~Strikethrough~~`
...
Items of an unordered list are prefixed with an asterisk `*`, hyphen `-`, or plus `+`. Increase the line spacing between bullet points by adding extra carriage returns and nest bullet points by prepending them with four blank spaces.
...
Code chunks are enclosed by three backticks `` ``` `` followed by curly parentheses `{}` containing comma-seperated parameters for that chunk (if any). 
...
1. Add a new R chunk by clicking in your notebook where you want to insert the chunk and clicking the *Insert* button on the toolbar. 
   ...
   Alternatively (and preferably) use the *Ctrl + Alt + I* keyboard shortcut.
...
````
```{r sepal width boxplot echo=FALSE, fig.height=4, fig.width=6} `r ''`
boxplot(Sepal.Width ~ Species, data = iris, col = hcl.colors(3))
```
````
