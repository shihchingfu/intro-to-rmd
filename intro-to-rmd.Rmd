...
RStudio includes all that you need to get started with creating an R Notebook. The steps below create a new (HTML) notebook with file extension `.Rmd` that's pre-filled with some basic R Markdown content.
...
`Inline code` styling is applied using backticks: `` `plot(iris)` ``
...
Below is an example of a chunk that produces a figure rather than numerical output, namely a boxplot of sepal widths from the `iris` dataset.
